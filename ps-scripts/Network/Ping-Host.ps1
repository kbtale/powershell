...
try {
	...
	foreach($ping in $tasks.Result) {
		if ($ping.Status -eq "Success") {
			...
			Write-Output "?? No reply from '$hostname' (IP $($ping.Address)) - check the connection or maybe the host is down."
			exit 1
		}
	}
	Write-Output "?? No reply from '$hostname' - check the connection or maybe the host is down."
	exit 1
} catch {
