...
.DESCRIPTION
    Retrieves the aggregate user status counts (Success, Failure, etc.) for a specifies device configuration profile in Microsoft Graph.

.PARAMETER Identity
    Specifies the ID of the device configuration profile.

.EXAMPLE
    PS> ./Get-MgmtGraphDeviceConfigurationUserStatusOverview.ps1 -Identity "config-id"
