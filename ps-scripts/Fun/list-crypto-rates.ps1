...
function ListCryptoRate { param([string]$Symbol, [string]$Name)
	...
	New-Object PSObject -Property @{ Timestamp = (Get-Date -Format "yyyy-MM-dd HH:mm:ss"); 'CRYPTOCURRENCY' = "1 $Name ($Symbol) ="; 'US$' = "$($rates.USD)"; '€' = "$($rates.EUR)"; 'CN¥' = "$($rates.CNY)"; 'JP¥' = "$($rates.JPY)" }
}
