...
<#
.SYNOPSIS
    DBSystems: Removes members from a specific database role

.DESCRIPTION
    Removes one or more members from a specified SQL Server database role. This script accepts a comma-separated list of members and returns the updated membership status as structured data.
...
.EXAMPLE
    PS> ./Remove-DatabaseRoleMember.ps1 -ServerInstance "localhost\SQLEXPRESS" -DBName "TestDB" -RoleName "db_datareader" -Members "User1"
