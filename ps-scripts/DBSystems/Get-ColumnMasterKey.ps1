...
<#
.SYNOPSIS
    DBSystems: Gets SQL Server database column master key objects

.DESCRIPTION
    Retrieves information about column master keys (CMK) defined within a SQL Server database. This script provides a standardized way to audit encryption metadata, returning structured data objects for each master key.
...
.PARAMETER KeyName
    Specifies the name of a specific column master key to retrieve

.PARAMETER ConnectionTimeout
    Specifies the time period to retry the command on the target server

.EXAMPLE
    PS> ./Get-ColumnMasterKey.ps1 -ServerInstance "localhost\SQLEXPRESS" -DatabaseName "SalesDB"
