...
<#
.SYNOPSIS
    DBSystems: Adds members to a specific SQL Server instance-level role

.DESCRIPTION
    Adds one or more members to a specified server-level role (instance role). This script accepts a comma-separated list of members and returns the updated membership status as structured data.
...
.EXAMPLE
    PS> ./Add-InstanceRoleMember.ps1 -ServerInstance "localhost\SQLEXPRESS" -RoleName "sysadmin" -Members "CONTOSO\User1"
