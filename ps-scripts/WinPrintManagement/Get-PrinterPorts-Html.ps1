...
<#
.SYNOPSIS
    Print Management: HTML report of printer ports
.DESCRIPTION
    Generates an HTML report of printer ports.
.PARAMETER ComputerName
    Computer to retrieve printer ports from
.PARAMETER AccessAccount
    Optional credential for remote access
.EXAMPLE
    PS> ./Get-PrinterPorts-Html.ps1 -ComputerName "PC01" | Out-File ports.html
...
Process {
    try {
        ...
        $result = Get-PrinterPort -CimSession $cim -ComputerName $ComputerName -ErrorAction Stop | Select-Object * | Sort-Object Name
