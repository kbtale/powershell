...
function TryEditor { param([string]$editor, [string]$path)
	try {
		...
		& $editor "$path"
		if ($lastExitCode -ne 0) { "?? Can't execute '$editor' - make sure it's installed and available"; exit 1 }
		exit 0 # success
	} catch { return }
}
...
"?? Sorry, no supported text editor found (Emacs, Helix, pico, nano, neovim, Notepad, vi, vim, Visual Studio Code, Wordpad)"
exit 1
