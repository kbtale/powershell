<#
.SYNOPSIS
	Copies a file, ignoring device I/O errors
.DESCRIPTION
	This PowerShell script copies the specified file even if it contains unreadable blocks caused by device I/O errors. The unreadable blocks will be replaced with zeros. The block size is determined by the buffer parameter.
...
.PARAMETER MaxRetries
	Amount of retry attempts for failed reads.
.PARAMETER Overwrite
	Overwrite destination file if it exists.
.EXAMPLE
	PS> ./copy-broken-file.ps1 -SourceFilePath 'bad_disk\file' -DestinationFilePath 'output' -MaxRetries 6
.CATEGORY Utilities
#>
