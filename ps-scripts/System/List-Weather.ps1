...
.EXAMPLE
	PS> ./list-weather.ps1
	TODAY Temp  ??mm  ??  ??km/h  ??UV  ??   ??km   Munich (Bayern, Germany)
...
function GetEmoji([string]$text) { 
	...
	"Light sleet"			{ return "??" }
	"Light sleet showers"		{ return "??" }
	"Light snow"			{ return "??" }
	"Light snow showers"		{ return "??" }
	"Moderate or heavy freezing rain"{return "??" }
	"Moderate or heavy sleet"	{ return "??" }
	"Moderate or heavy rain shower" { return "??" }
	"Moderate or heavy rain in area with thunder" { return "??" }
	"Moderate or heavy snow showers"{ return "??" }
	"Moderate or heavy snow in area with thunder" { return "??" }
	"Moderate rain"			{ return "??" }
	"Moderate rain at times"	{ return "??" }
	"Moderate snow"			{ return "??" }
	"Mist"				{ return "?? " }
	"Overcast"			{ return "??" }
	"Partly cloudy"			{ return "??" }
	"Patchy heavy snow"		{ return "??" }
	...
	"Patchy light snow"		{ return "??" }
	"Patchy moderate snow"		{ return "??" }
	"Patchy rain possible"  	{ return "??" }
	"Patchy rain nearby"		{ return "??" }
	"Patchy sleet nearby"		{ return "??" }
	"Patchy snow nearby"		{ return "??" }
	"Patchy snow possible"  	{ return "??" }
	"Sunny"				{ return "??" }
	"Thundery outbreaks possible"	{ return "??" }
	"Thundery outbreaks in nearby"	{ return "??" }
	default				{ return "?" }
	}
}
...
try {
	...
	foreach($hourly in $weather.weather.hourly) {
		...
		if ($hour -eq 0) {
			if ($day -eq 0) {
				Write-Host "`nTODAY  Temp  ??mm    ?? ??km/h   ??  ??UV ?? km  $area ($region, $country)" -foregroundColor green
			} elseif ($day -eq 1) {
