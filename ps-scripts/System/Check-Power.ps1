	...
	if ($IsLinux) {
		...
		if ($details.PowerLineStatus -eq "Online") {
			...
			if (($remaining -eq 0) -and ($percent -ge 60)) {
				...
				$reply = "?? Battery LOW ($percent%, $($remaining)min remaining, power scheme is '$powerScheme')"
			} elseif ($remaining -le 30) {
				$reply = "?? Battery low ($percent%, $($remaining)min remaining, power scheme is '$powerScheme')"
			} elseif ($percent -lt 10) {
				$reply = "?? Battery $percent% only with $($remaining)min remaining (power scheme is '$powerScheme') "
			} elseif ($percent -ge 90) {
