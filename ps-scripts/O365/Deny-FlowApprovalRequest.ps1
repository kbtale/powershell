...
<#
.SYNOPSIS
    PowerApps: Rejects a flow approval request
.DESCRIPTION
    Rejects a pending flow approval request with optional comments.
...
.PARAMETER ApiVersion
    API version to call
.EXAMPLE
    PS> ./Deny-FlowApprovalRequest.ps1 -PACredential $cred -ApprovalId "abc" -ApprovalRequestId "xyz" -EnvironmentName "default" -Comments "Rejected"
.CATEGORY O365
#>
...
Process {
    try {
        ...
        $result = Deny-FlowApprovalRequest @cmdArgs -ErrorAction Stop | Select-Object *
